...
### Creating Batch pool from template
...
Pool template parameters description:
 ...
 - *tasks per node* - Maximum number of tasks allowed to run in parallel per node. The default value is 16 (equal to the amount of cores on H16 VM). You may need to reduce it so that the anticipated size of input HBASE and output JSON data of all tasks running on the node fits on the VM local disk (2 TB).
 
Provide the pool parameter values or accept the defaults and click *Create and close* button.
5. The pool deployment process will start. You should now see and manage your pool in *Pools* tab: 
![pool-deployment](screenshots/pool-deployment.png)

### Preparing file-group containers for running convertion jobs
...
You are now ready to run the hbase-2-json conversion jobs. 


### Running the job from template  
...
![task-stdout](screenshots/task-stdout.png)
...
### Deallocating billable compute resources
